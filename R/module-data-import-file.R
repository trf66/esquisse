...
#'
dataImportFileUI <- function(id, dismissOnValidate = FALSE, selectVars = TRUE, coerceVars = FALSE) {
  ns <- NS(id)
  ...
  tagList(
    ...
    tags$h2("Importer un fichier local"),
    fileInput(
      inputId = ns("file"), 
    ...
    tags$div(
      ...
      tags$div(
        ...
        tags$b("Aucun fichier"), "Format : .rds, .txt, .csv, .xls, .xlsx, .sas7bdat, .sav, ..."
      )
    ),
...
dataImportFileServer <- function(input, output, session, data = NULL, name = NULL, 
  ...
  if (!requireNamespace(package = "rio", quietly = TRUE))
    message("Package 'rio' est recquis pour cette foction")
    ...
    if ("try-error" %in% class(imported) || NROW(imported) < 1) {
      ...
      insertUI(
        ...
        ui = tags$div(
          ...
          tags$b("Failure"), "le fichier ne peut pas être lu"
        )
      )
