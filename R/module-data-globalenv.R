...
#'
dataGlobalEnvUI <- function(id, dismissOnValidate = TRUE, selectVars = TRUE, coerceVars = coerceVars) {
  ns <- NS(id)
  ...
  info_dfs <- lapply(
    ...
    FUN = function(x) {
      tmp <- get_df(x)
      sprintf("%d obs. de %d variables", nrow(tmp), ncol(tmp))
    }
  )
  ...
  tagList(
    ...
    tags$h2("Importation des données"),  #des données locale sur le serveur
    pickerInput(
      inputId = ns("data"),
      label = "Choisissez vos données",
      choices = dfs, 
      width = "100%",
      options = list(title = "Liste des données disponibles..."),
    ...
    tags$div(
      ...
      tags$div(
        ...
        tags$b("Aucune donnée sélectionnée"), #"Utiliser un data.frame de l'envt utilisateur"
      )
    ),
...
dataGlobalEnvServer <- function(input, output, session, data = NULL, name = NULL, 
    ...
    if ("try-error" %in% class(imported) || NROW(imported) < 1) {
      ...
      insertUI(
        ...
        ui = tags$div(
          ...
          tags$b("Ooops"), "Une erreur s'est produite :("
        )
      )
      ...
      insertUI(
        ...
        ui = tags$div(
          ...
          tags$b("Success"), 
          sprintf("%s obs. de %s variables importées", 
  ...
  observeEvent(sv$selected_vars, {
    if (length(sv$selected_vars) > 0) {
      toggleInput(inputId = ns("validate"), enable = TRUE)
      coerce_data$data <- select_data$data[, sv$selected_vars, drop = TRUE]
    } else {
      toggleInput(inputId = ns("validate"), enable = FALSE)
    }
  }, ignoreNULL = FALSE)
  
  coerced_data <- callModule(module = coerceServer, id = "selected", data = coerce_data)
