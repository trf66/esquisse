...
esquisserUI <- function(id, header = FALSE,
  ...
  box_title <- tags$div(
    ...
    tags$div(
      class = "pull-right",
      miniTitleBarButton(inputId = ns("close"), label = "FERMER")
    ),
  ...
  addin <- miniPage(
    ...
    layoutAddin(
      top_left = htmltools::tagList(
        htmltools::tags$div(
          style = if (isTRUE(choose_data) & !isTRUE(header)) "padding: 10px;" else "padding: 8px; height: 100%;",
          dropInput(
            inputId = ns("geom"),
      ...
      top_right = dragulaInput(
       ...
       # targetsLabels = c("Abscisse", "Ordonnée"),
        #targetsIds = c("xvar", "yvar"),
        targetsLabels = c("Xvar", "Yvar", "Ajustement", "Couleur", "Taille", "Groupe", "Multi-pan."), 
      ...
      main = htmltools::tags$div(
       # style = "margin-top: 4px; padding-bottom: 20px",#; height: 100%;",
        style = "margin-top: 10px; padding-bottom: 20px; height: 100%;", #padding-bottom: 10px;
        tags$div(
          style = "position: absolute; right: 0; top: 60px; font-weight: bold; z-index: 1000;",
          #style = "position: absolute; right: 0; top: 40px; font-weight: bold; z-index: 1000;",
          prettyToggle(
        ...
        br(), br(), #lignerajoutée pour décaler le graph vers le bas
        shiny::plotOutput(outputId = ns("plooooooot"), width = "100%", height = "100%") )),  
        br(), br(), br(),   #lignerajoutée pour décaler le menu vers le bas
...
#' @export
esquisseContainer <- function(width = "100%", height = "450px", fixed = FALSE) {
  force(width)
  force(height)
  force(fixed)
  function(...) {
