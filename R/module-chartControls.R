...
chartControlsUI <- function(id, insert_code = FALSE, disable_filters = FALSE) {
  ...
  tags$div(
    ...
    tags$style(sprintf(
      "#%s .sw-dropdown-in {margin: 30px 8px 8px 10px !important; padding: 0 !important;}",
      #"#%s .sw-dropdown-in {margin: 8px 0 8px 10px !important; padding: 0 !important;}",  #v orig
...
chartControlsServer <- function(input, output, session, 
  ...
  output$export_png <- downloadHandler(
    filename = function() {
      paste0("graphique_", format(Sys.time(), format = "%Y%m%dT%H%M%S"), ".png")
    },
    content = function(file) {
      pngg <- try(ggsave(filename = file, plot = ggplot_rv$ggobj$plot, width = 12, height = 8, dpi = "retina"))
      if ("try-error" %in% class(pngg)) {
        shiny::showNotification(ui = "Echec de l'export vers PNG...", type = "error")
      }
    }
  )
  output$export_ppt <- downloadHandler(
    filename = function() {
      paste0("graphique_", format(Sys.time(), format = "%Y%m%dT%H%M%S"), ".pptx")
    },
    content = function(file) {
      if (requireNamespace(package = "rvg") & requireNamespace(package = "officer")) {
        gg <- ggplot_rv$ggobj$plot
        ppt <- officer::read_pptx()
        ppt <- officer::add_slide(ppt, layout = "Titre et contenu", master = "Office Theme")
        ...
        if ("try-error" %in% class(ppt)) {
          shiny::showNotification(ui = "Echec de l'export vers PowerPoint...", type = "error")
        } else {
        ...
        warn <- "Packages 'officer' et 'rvg' nécéssaire opour utiliser cette fonctionnalité"
...
controls_labs <- function(ns) {
  tags$div(
    class = "form-group",
    textInput(inputId = ns("labs_title"), placeholder = "Titre", label = NULL),
    textInput(inputId = ns("labs_subtitle"), placeholder = "Sous-titre", label = NULL),
    textInput(inputId = ns("labs_caption"), placeholder = "Annotation", label = NULL),
    textInput(inputId = ns("labs_x"), placeholder = "Label axe X", label = NULL),
    textInput(inputId = ns("labs_y"), placeholder = "Label axe Y", label = NULL),
    ...
    tags$div(
      ...
      textInput(inputId = ns("labs_color"), placeholder = "Couleur label", label = NULL)
    ),
    tags$div(
      id = ns("controls-labs-size"), style = "display: none;",
      textInput(inputId = ns("labs_size"), placeholder = "Taille label", label = NULL)
    )
  )
}
...
controls_appearance <- function(ns) {
  ...
  tagList(
    ...
    pickerInput(
      ...
      options = list(size = 16, container = "body"),
      width = "100%"
    ),
    tags$script(
  ...
  tagList(
    tags$div(
      ...
      materialSwitch(
        ...
        label = "Lissage des données (smooth)",
        right = TRUE, 
        status = "primary"
      ),
    ),
...
#    ),
 ...
 #     id = ns("controls-scale-trans-x"), style = "display: none;",
  #    selectInput(
  #      inputId = ns("transX"), 
  #      label = "X-Transformation de l'axe:",
   #    selected = "identity", 
 ...
 #  tags$div(
  ...
  #    selectInput(
  #      inputId = ns("transY"), 
  #      label = "Y-Transformation de l'axe:",
  #      selected = "identity", 
  #      choices = scales_trans
   #   )
  #  ),
...
controls_code <- function(ns, insert_code = FALSE) {
  tagList(
    tags$button(
      ...
      "Copier", `data-clipboard-target` = paste0("#", ns("codeggplot"))
    ...
    if (insert_code) {
      actionLink(
        ...
        label = "Insertion du code R",
        icon = icon("arrow-circle-left")
      )
    },
    tags$br(),
    tags$b("Exporter au format:"),
    tags$br(),
    tags$div(
...
select_geom_controls <- function(x, geoms) {
  if ("bar" %in% geoms & x %in% c("auto", "bar")) {
    ...
    "point"
  } else if ("ligne" %in% geoms & x %in% c("auto", "line")) {
    "line"
