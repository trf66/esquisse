...
dragulaInput <- function(inputId, sourceLabel, targetsLabels, 
  ...
  target_list$style <- "height: 70%; font-size: 0; " 
  target_list$cellArgs <- list(style = "height: 40px; padding: 0; margin-right: 0.5%;") #"height: 70%; espacement des 2 boites X- Y / 50% hauteur cellule
  ...
  tagList(
    ...
    tags$div(
      ...
      tags$div(
        ...
        tags$div( # boite des 2 logos à gauche
          id = paste(inputId, "source", sep = "-"), 
          style = "margin: 2px; width: 100%; min-height:40px; margin-right: 4px;", #logo upload  mini heigh=40 ; #Miniheight 30
          makeDragulaChoices(inputId = inputId, 
                             args = args, status = status, badge = badge)
        )
      ),
